...
public class KingdomTurnAction {
    ...
    public KingdomTurnAction(KingdomEntity kingdom, IKingdomInteractor kingdomInteractor, GameConfig gameConfig, KingdomDetailsProvider kingdomDetailsProvider)
    {
    ...
    private void fireProfessionalsBasedOnRatios(int professionalsToFire)
    {
        ...
        for (var unit : UnitName.values())
        {
            ...
            kingdom.units.subtractAvailableCount(unit, toFire);
    ...
    private void fireProfessionalsBasedOnCount(int professionalsToFire)
    {
        ...
        while (professionalsToFire > 0)
        {
            for (var unit : UnitName.values())
            {
                ...
                if (count > 0)
                {
                    kingdom.units.subtractAvailableCount(unit, 1);
    ...
    private void proffesionalsLeavingDueToInsufficientBuildings()
    {
        for (var unit : UnitName.values())
        {
            ...
            var capacity = kingdom.getBuildingCapacity(building, config);
            // TODO how about firing while unit is mobile?
            var count = kingdom.getUnits().getTotalCount(unit);
            if (count > capacity)
            {
                var leaving = count - capacity;
                kingdom.units.subtractAvailableCount(unit, leaving);
    ...
    private double eatFood()
    {
        ...
        int foodUpkeep = kingdomDetailsProvider.getFoodUpkeep(kingdom);
    ...
    private void doProduction(double nourishmentProductionFactor, int weaponsProductionPercentage)
    {
        ...
        for (var resourceType : ResourceName.productionResourceNames())
        {
            ...
            {
                int neededIron = kingdomDetailsProvider.getIronUpkeep(kingdom, nourishmentProductionFactor);
    ...
    private double getSpecialBuildingBonus(ResourceName resourceName)
    {
        ...
        var summedLevels = kingdom.getSpecialBuildings().stream()
                .filter(b -> b.buildingType == specialBuildingType)
                .mapToInt(specialBuilding -> specialBuilding.level).sum();
    ...
    private int getHousingCapacity()
    {
        ...
        int housesCapacity = kingdom.buildings.getCount(BuildingName.house) * config.buildingCapacity().getCapacity(BuildingName.house);
